...
{
    if (state == PipeState::WaitingToConnect || state == PipeState::Disconnected || state == PipeState::Closed || handle
        .IsInvalid())
    {
        throw InvalidOperationException();
    }
...
{
    if (state == PipeState::WaitingToConnect || state == PipeState::Disconnected || state == PipeState::Broken || state
        == PipeState::Closed || handle.IsInvalid())
    {
        throw InvalidOperationException();
    }
}
